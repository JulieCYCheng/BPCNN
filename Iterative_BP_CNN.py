...
def simulation_colored_noise(linear_code, top_config, net_config, simutimes_range, target_err_bits_num, batch_size):
    ...
    if top_config.urllc_intf:
        ber_file = format('%s_urllc' % ber_file)
    ...
    for SNR in SNRset:
        ...
        for ik in range(0, max_batches):
            ...
            x_bits, _, s_mod, ch_noise, y_receive, LLR, intf_label = lbc.encode_and_transmission(G_matrix, SNR, real_batch_size, noise_io, top_config, rng)
...
def generate_noise_samples(linear_code, top_config, net_config, train_config, bp_iter_num, net_id_data_for,
                           generate_data_for, noise_io, model_id):
    ...
    for ik in range(0, total_batches):  # number of batches
        for SNR in SNRset_for_generate_training_data:
            x_bits, _, _, channel_noise, y_receive, LLR, intf_label = lbc.encode_and_transmission(G_matrix, SNR, batch_size_each_SNR, noise_io, top_config)
...
def analyze_residual_noise(linear_code, top_config, net_config, simutimes, batch_size):
    ...
    model_id_str = np.array2string(model_id, separator='_', formatter={'int': lambda d: "%d" % d})
    model_id_str = model_id_str[1:(len(model_id_str) - 1)]
    loss_file_name = format("%sresidual_noise_property_netid%d_model%s.txt" % (
    net_config.residual_noise_property_folder, net_id_tested, model_id_str))
    ...
    for SNR in SNRset:
        ...
        for ik in range(0, max_batches):
            ...
            if ik == max_batches - 1 and residual_times != 0:
                real_batch_size = residual_times
            x_bits, _, s_mod, channel_noise, y_receive, LLR, intf_label = lbc.encode_and_transmission(G_matrix, SNR, real_batch_size, noise_io, top_config)
